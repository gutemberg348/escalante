...
export default function Seniority() {
  ...
  const { data = { items: [], turn: null, markingMode: 'OPEN' } } = useQuery({ queryKey: ['seniority'], queryFn: () => api.get('/seniority').then((response) => response.data) });
  ...
  return <>
    ...
    <section className={`panel marking-sequence-card ${data.turn ? '' : 'open-marking-mode'}`}><div className="sequence-copy"><span className="eyebrow">{data.turn ? 'VEZ ATUAL' : 'MODO DE MARCAÇÃO'}</span><h2>{data.turn ? `${data.turn.rank} ${data.turn.operational_name}` : 'Marcação livre'}</h2><p>{data.turn ? `Pode marcar até ${new Date(data.turn.deadline_at).toLocaleString('pt-BR', { dateStyle: 'short', timeStyle: 'short' })}.` : 'A fila está encerrada. Qualquer militar ativo e autorizado pode marcar as vagas disponíveis, sem ordem de antiguidade.'}</p></div><div className="connection-actions">{data.turn ? <><button type="button" className="secondary-button" onClick={() => notify.mutate()} disabled={notify.isPending}>Enviar vagas agora</button><button type="button" className="secondary-button" onClick={() => close.mutate()} disabled={close.isPending}>Encerrar e liberar para todos</button></> : <button type="button" onClick={() => start.mutate()} disabled={start.isPending}>Iniciar pelo horário atual</button>}<button type="button" className="secondary-button reset-turn-button" onClick={resetTurn} disabled={reset.isPending}>↺ Reiniciar fila</button></div>{error && <p className="error sequence-error">{error.response?.data?.message || 'Não foi possível atualizar a sequência.'}</p>}{start.isSuccess && <p className="success sequence-error">{start.data?.data?.message || 'Fila iniciada pelo primeiro prazo ainda válido.'}</p>}{close.isSuccess && !data.turn && <p className="success sequence-error">Fila encerrada. A marcação está liberada para todos os militares ativos e autorizados.</p>}{reset.isSuccess && <p className="success sequence-error">{reset.data?.data?.message || 'Fila reiniciada pelo primeiro prazo ainda válido.'} As marcações anteriores foram preservadas.</p>}</section>
