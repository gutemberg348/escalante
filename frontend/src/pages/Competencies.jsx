...
export default function Competencies() {
  ...
  const generateNext = useMutation({
    mutationFn: () => api.post('/competencies/generate-next'),
    onSuccess: () => queryClient.invalidateQueries({ queryKey: ['competencies'] })
  });
  const generated = generateNext.data?.data;
  return <>
    <header className="page-header"><div><span className="eyebrow">PLANEJAMENTO MENSAL</span><h1>Meses da escala</h1><p>O próximo mês mantém automaticamente o ciclo ordinário de 1 serviço por 4 folgas.</p></div><button type="button" onClick={() => generateNext.mutate()} disabled={generateNext.isPending}>{generateNext.isPending ? 'Gerando…' : 'Gerar próximo mês'}</button></header>
    {generateNext.isSuccess && <section className="panel"><p className="success"><strong>{generated.item.name}</strong> gerado: {generated.generation.ordinaryDutyDays} serviços ordinários, {generated.generation.assignmentsCreated} turnos novos. Inativos, afastados ou não autorizados foram ignorados.</p>{generated.generation.conflictDays.length > 0 && <p className="error">Existem {generated.generation.conflictDays.length} dias com conflito; revise a escala no gerenciamento.</p>}</section>}
    {generateNext.isError && <p className="error">{generateNext.error.response?.data?.message || 'Não foi possível gerar o próximo mês.'}</p>}
