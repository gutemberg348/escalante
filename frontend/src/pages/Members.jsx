...
export default function Members() {
  ...
  const [vacationConfirmation, setVacationConfirmation] = useState(null);
  ...
  const captureVacationConfirmation = (error, variables, kind) => {
    if (error.response?.data?.code !== 'VACATION_EXTRAS_CONFIRMATION') return;
    const member = data.find(item => Number(item.id) === Number(variables.id)) || variables;
    setVacationConfirmation({ kind, variables, member, impact: error.response.data.impact });
  };
  ...
  const save = useMutation({
    ...
    onSuccess: async response => { await handleStatusEffect(response); setForm(empty); setEditing(null); setVacationConfirmation(null); },
    onError: (error, variables) => captureVacationConfirmation(error, variables, 'SAVE')
  });
  const changeStatus = useMutation({
    mutationFn: ({ id, status, vacation_extra_action }) => api.patch(`/members/${id}`, { operational_status: status, active: status !== 'INACTIVE', competency_id: competencyId ? Number(competencyId) : undefined, vacation_extra_action }),
    onSuccess: async response => { await handleStatusEffect(response); setVacationConfirmation(null); },
    onError: (error, variables) => captureVacationConfirmation(error, variables, 'STATUS')
  });
  ...
  const confirmVacation = action => {
    if (!vacationConfirmation) return;
    const variables = { ...vacationConfirmation.variables, vacation_extra_action: action };
    if (vacationConfirmation.kind === 'SAVE') save.mutate(variables);
    else changeStatus.mutate(variables);
  };
  ...
  return <><header className="page-header"><div><span className="eyebrow">CADASTRO OPERACIONAL</span><h1>Efetivo</h1><p>Edite dados, situação e o limite mensal individual de horas.</p></div><button className="page-action" onClick={() => { setEditing({}); setForm(empty); }}>+ Novo militar</button></header>
    ...
    <section className="panel member-hours-filter"><div><span className="eyebrow">ESCALA SELECIONADA</span><h2>Competência para situação e horas</h2><p>Férias e retorno ao status Ativo alteram esta escala. Cada serviço extra confirmado vale 12h.</p></div><label>Mês / competência<select value={competencyId} onChange={event => setCompetencyId(event.target.value)}><option value="">Selecione uma escala gerada</option>{generatedCompetencies.map(competency => <option value={competency.id} key={competency.id}>{competency.name}</option>)}</select></label></section>
    ...
    {vacationConfirmation && <div className="modal-backdrop" role="presentation"><section className="edit-modal member-delete-modal" role="dialog" aria-modal="true" aria-label="Confirmar férias"><div className="modal-header"><div><span className="eyebrow">CONFIRMAR ALTERAÇÃO</span><h2>Férias de {vacationConfirmation.member.rank} {vacationConfirmation.member.operational_name}</h2></div><button className="close-button" onClick={() => setVacationConfirmation(null)} aria-label="Fechar">×</button></div><p>Na escala selecionada existem <b>{vacationConfirmation.impact.ordinaryAssignments}</b> marcações ordinárias e <b>{vacationConfirmation.impact.extraordinaryAssignments}</b> extras. Os ordinários serão removidos e as posições serão reorganizadas.</p><p>O que deseja fazer com os serviços extras?</p><div className="form-actions"><button type="button" className="secondary-button" onClick={() => setVacationConfirmation(null)}>Cancelar</button><button type="button" className="secondary-button" onClick={() => confirmVacation('KEEP')} disabled={save.isPending || changeStatus.isPending}>Manter extras</button><button type="button" className="danger-button" onClick={() => confirmVacation('REMOVE')} disabled={save.isPending || changeStatus.isPending}>Retirar extras</button></div></section></div>}
