...
const forms = [
  ...
  ...['20 noite', '20,noite', '20 à noite', '20 noturno', '20 noturna', '20 N', '20n', '20noite',
    '20 pela noite, por favor!', 'noite no dia 20', '20 das 19h às 07h', '20 19:00-07:00', '20 noite 12 horas', '20 noite?']
...
for (const input of ['20 dia ou noite', '20 12h', '20, tarde', 'manhã dia 20', '20 08h às 20h', '20 08 as 20',
  ...
  '20 dia; 32 noite', '20 dia; 21 notie', '20, notie', 'não marque dia 20',
...
for (const [input, expected] of [
  ['12h "dia" hoje?', [choice(14, day)]],
  ['hoje de dia', [choice(14, day)]],
  ...
  assert.deepEqual(parseMarkingRequest(input, { month: 9, year: 2026, today: '2026-09-14' }), { choices: expected, error: null });
});

test('today outside the active month is refused', () => {
  const result = parseMarkingRequest('12h dia hoje?', { month: 10, year: 2026, today: '2026-09-14' });
  assert.match(result.error, /não pertence ao mês ativo/i);
  assert.deepEqual(result.choices, []);
});
